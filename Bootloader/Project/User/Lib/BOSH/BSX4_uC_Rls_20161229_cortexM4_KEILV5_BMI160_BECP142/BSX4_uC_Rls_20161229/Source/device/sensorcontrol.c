...
/*According to the schematic of shuttle board, the I2C and SPI Bus are using the same GPIO.
	...
	bmi160_m.burst_read = BST_SENSOR_I2C_READ;
	...
	delaym_local(10);
	bmi160_set_accel_under_sampling_parameter(BMI160_US_DISABLE); //0 normal node,1 lowpower mode cond+acc_nom
	delaym_local(10);
	bmi160_set_accel_range(BMI160_ACCEL_RANGE_2G);
	delaym_local(10);
	bmi160_set_accel_output_data_rate(BMI160_GYRO_OUTPUT_DATA_RATE_100HZ,BMI160_ACCEL_NORMAL_AVG4);//set acc phy ODR
	delaym_local(10);
	bmi160_set_accel_bw(BMI160_ACCEL_NORMAL_AVG4);
	delaym_local(10);
	bmi160_set_command_register(0x11); // Sets the PMU mode for the Accelerometer to normal
	delaym_local(50);
	bmi160_set_gyro_range((uint8_t)0);
	delaym_local(10);
	bmi160_set_gyro_output_data_rate(BMI160_GYRO_OUTPUT_DATA_RATE_100HZ);//set gyro phy ODR
	delaym_local(10);
	bmi160_set_gyro_bw(BMI160_GYRO_NORMAL_MODE);
	delaym_local(10);
	bmi160_set_command_register(0x15); // Sets the PMU mode for the Gyroscope to normal
	delaym_local(90);
