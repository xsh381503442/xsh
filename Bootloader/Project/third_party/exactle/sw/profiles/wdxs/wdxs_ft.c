 ...
 *  \file   wdxs_ft.c
 *        
 *  \brief  Wicentric Data Exchange profile implementation - File Transfer.
 *
...
static bool_t wdxsFileRead(uint16_t handle, uint32_t offset, uint32_t *pReadLen, uint8_t *pData)
{
  ...
  WsfEfsGet(handle, offset, pData, *pReadLen);
...
{
  uint32_t availableSize = WsfEfsGetFileMaxSize(handle) - WsfEfsGetFileSize(handle);
