 ...
 *  \file   wdxs_main.c
 *        
 *  \brief  Wicentric Data Exchange profile implementation.
 *
 *          $Date: 2016-06-21 14:35:46 -0700 (Tue, 21 Jun 2016) $ 
 *          $Revision: 7474 $
 *  
...
static void wdxsFormatFileResource(uint8_t *pData, wsfEfsHandle_t handle)
{
  ...
  WstrnCpy(pData, WsfEfsGetFileName(handle), WSF_EFS_NAME_LEN);
  WstrnCpy(pData+WSF_EFS_NAME_LEN, WsfEfsGetFileVersion(handle), WSF_EFS_VERSION_LEN);
}
...
void WdxsHandlerInit(wsfHandlerId_t handlerId)
{
  ...
  handle = WsfEfsAddFile(WDXS_FLIST_MAX_LEN, WDXS_RAM_MEDIA, &attr, WSF_EFS_FILE_OFFSET_ANY);
  WSF_ASSERT(handle == WDXS_FLIST_HANDLE);
}