 ...
 *
 *          $Date: 2016-03-11 14:00:41 -0800 (Fri, 11 Mar 2016) $
 *          $Revision: 6305 $
 *
 ...
 *
 *  \param  pBuf1   Buffer to copy.
 *  \param  pBuf2   Buffer to hold reversed copy.
 *  \param  len     Size of pBuf1 and pBuf2 in bytes.
 *
 *  \return None.
 */
/*************************************************************************************************/
static void cmacReverseCpy(uint8_t *pBuf1, const uint8_t *pBuf2, uint8_t len)
{
  int8_t i;

  for (i=0; i<len; i++)
  {
    pBuf1[SEC_BLOCK_LEN-1-i] = pBuf2[i];
  }
}
 ...
 */
/*************************************************************************************************/
static void cmacReverse(uint8_t *pBuf, uint8_t len)
{
  uint8_t i, temp;
...
static void secCmacProcessBlock(secQueueBuf_t *pBuf)
{
  ...
  {
    cmacReverseCpy(buf, pMn, remaining);
  ...
  else
  {
    ...
    cmacReverseCpy(buf, pMn, SEC_BLOCK_LEN);
  }
...
static void secCmacComplete(secQueueBuf_t *pBuf)
{
  ...
  cmacReverse(pBuf->ciphertext, SEC_CMAC_KEY_LEN);
               ...
               uint16_t param, uint8_t event)
{
  ...
  if ((pBuf = WsfMsgAlloc(bufSize)) != NULL)
  {
    ...
    /* Copy key */
    cmacReverseCpy(pCmacCb->key, pKey, SEC_CMAC_KEY_LEN);
