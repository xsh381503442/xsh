...
extern FILE *fp;
float OFFSET_X,OFFSET_Y,OFFSET_Z,MEAN_X,MEAN_Y,MEAN_Z;
...
int16 AKFS_HOBUF_SIZE_ADD;
int16 AKFS_HOBUF_SIZE_ADD2 = 0;
...
int16 AKFS_AOC(				/*!< (o) : calibration success(AKFS_SUCCESS), failure(AKFS_ERROR) */
	...
	// check distance of new data and old data
...
//	if (num < 4) {
	...
	if (0 != From4Points2Sphere(fourpoints, &tempho, &haocv->hraoc)) {
		AOC_WRITESTRING("================AOC  From4Points2Sphere error\n ");		
		return AKFS_ERROR;
	}
	...
	for (i = 0; i < 4; i++) {
		for (j = (i+1); j < 4; j++) {
			...
			if ((tempf < haocv->hraoc) || (tempf < AKFS_HR_TH)) {
				AOC_WRITESTRING("===============AOC  AKFS_HR_TH error\n ");	
				return AKFS_ERROR;
			}
		}
	}
	...
	AKFS_HOBUF_SIZE_ADD2++;
	AOC_PRINTF("&&&&&&&&&&&&&&&&&&&&&&&======AOC  update offset buffer n=%d\n ",AKFS_HOBUF_SIZE_ADD2);	
	...
	if (CheckInitFvec(&haocv->hobuf[AKFS_HOBUF_SIZE-1]) == 1) {
		AOC_WRITESTRING("============AOC  AKFS_HOBUF_SIZE error\n ");	
		return AKFS_ERROR;
	}
	#else
		/* Check Init */
	if (CheckInitFvec(&haocv->hobuf[AKFS_HOBUF_SIZE_MIN-1]) == 1) {
		AOC_WRITESTRING("============AOC  AKFS_HOBUF_SIZE error\n ");	
		AKFS_HOBUF_SIZE_ADD=0;
		return AKFS_ERROR;
	}	
	#endif

	if(AKFS_HOBUF_SIZE_ADD==0)
		AKFS_HOBUF_SIZE_ADD = AKFS_HOBUF_SIZE_MIN;
	else
		{
		AKFS_HOBUF_SIZE_ADD++;
		if(AKFS_HOBUF_SIZE_ADD>=AKFS_HOBUF_SIZE)
			AKFS_HOBUF_SIZE_ADD=AKFS_HOBUF_SIZE;
		}
	...
	AOC_PRINTF("haocv->hraoc=%f  tempf=%f   AKFS_HOBUF_SIZE_ADD=%d\n ",haocv->hraoc,tempf,AKFS_HOBUF_SIZE_ADD);
	MeanVar(haocv->hobuf,/* AKFS_HOBUF_SIZE*/AKFS_HOBUF_SIZE_ADD, &mean, &var);
//	MeanVar_AVG(haocv->hobuf,/* AKFS_HOBUF_SIZE*/AKFS_HOBUF_SIZE_ADD, &mean, &var);
	if ((var.u.x >= tempf) || (var.u.y >= tempf) || (var.u.z >= tempf)) {
		AOC_WRITESTRING("AOC  Check ovar  error   ");	
		AOC_PRINTF("var.u.x=%f  var.u.y=%f  var.u.z=%f\n ",var.u.x,var.u.y,var.u.z);
		if(var.u.x>100 || var.u.y>100 || var.u.z >100)
		{
			AOC_WRITESTRING("AOC oooooooooo\n");	
		}
		return AKFS_ERROR;
	}
	...
	//	fprintf(fp," 6983 OFFSET_MEAN_YPR_YAW  ,%6.2f,%6.2f,%6.2f,%6.2f,%6.2f,%6.2f,",
	//			haocv->hobuf[0].u.x,haocv->hobuf[0].u.y,haocv->hobuf[0].u.z,	mean.u.x,mean.u.y,mean.u.z);

	OFFSET_X=haocv->hobuf[0].u.x;
	OFFSET_Y=haocv->hobuf[0].u.y;
	OFFSET_Z=haocv->hobuf[0].u.z;
