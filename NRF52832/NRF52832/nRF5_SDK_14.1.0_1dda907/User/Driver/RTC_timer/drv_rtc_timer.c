...
#define RTC2_IRQ_PRI            APP_TIMER_CONFIG_IRQ_PRIORITY               /**< Priority of the RTC1 interrupt (used for checking for timeouts and executing timeout handlers). */
...
typedef struct
{
    ...
    drv_rtc_timer_mode_t            mode;                                       /**< Timer mode. */
    drv_rtc_timer_timeout_handler_t p_timeout_handler;                          /**< Pointer to function to be executed when the timer expires. */
...
STATIC_ASSERT(sizeof(timer_node_t) == DRV_RTC_TIMER_NODE_SIZE);
...
static bool                          m_rtc2_reset;                              /**< Boolean indicating if RTC1 counter has been reset due to last timer removed from timer list during the timer list handling. */
...
ret_code_t drv_rtc_timer_init(void)
{
    ...
    rtc2_init(APP_TIMER_CONFIG_RTC_FREQUENCY);
